          ...
          <div className="space-y-4 w-full">
            {PROJECTS.map((project, idx) => (
              <button key={project.id} onClick={() => setActiveProject(idx)}
                className={`w-full text-left p-4 rounded-xl border transition-all duration-300 ${activeProject === idx ? 'bg-zinc-900/90 backdrop-blur-sm shadow-lg' : 'bg-zinc-900/60 backdrop-blur-sm border-zinc-700/50 hover:border-zinc-600'}`}
