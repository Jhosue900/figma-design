...
const ContactMktDigital = () => {
  ...
  const contactInfo = [
    { icon: <MessageCircle size={20} />, label: 'Whatsapp', value: '+52 1 33 1385 7143', href: 'https://wa.me/5213313857143?text=Hola,%20quiero%20más%20información%20sobre%20marketing%20digital.', color: '#25D366' },
  ...
  return (
    <section id="contacto" className="w-full bg-transparent py-16 px-4 sm:px-6 lg:px-8 font-montserrat text-white">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-5xl font-aston tracking-tight mb-4 leading-tight text-white">
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-start">
          <div className="w-full lg:w-2/5 flex flex-col gap-8">
            <div>
              <h3 className="text-xl md:text-2xl font-aston tracking-tight mb-4 leading-tight">
            ...
            <div className="flex flex-col gap-5">
              {contactInfo.map((item, index) => (
                <a key={index} href={item.href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 group">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 border transition-all duration-300 group-hover:scale-110"
                    style={{ borderColor: item.color, color: item.color, backgroundColor: `${item.color}15` }}>
                    {item.icon}
                  </div>
                  <div>
                    <p className="text-white/60 text-xs font-light mb-0.5">{item.label}</p>
          ...
          <div className="w-full bg-neutral-950 border p-5 border-neutral-800 rounded-2xl lg:w-3/5">
            {sent ? (
              <div className="flex flex-col items-center justify-center h-full gap-6 py-20 text-center">
                ...
                <p className="text-white/50 text-sm max-w-xs">Nos pondremos en contacto contigo muy pronto. ¡Gracias por escribirnos!</p>
                <button
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                <div className="flex flex-col gap-1.5">
                  <label className="text-white/40 text-xs font-medium uppercase tracking-widest">Nombre <span className="text-[#c5362e]">*</span></label>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-white/40 text-xs font-medium uppercase tracking-widest">Email <span className="text-[#c5362e]">*</span></label>
                  <input type="email" name="email" required placeholder="Correo electrónico" value={formData.email} onChange={handleChange}
                    className="bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3 text-white text-sm placeholder-white/20 outline-none transition-all duration-300 focus:border-neutral-600" />
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-white/40 text-xs font-medium uppercase tracking-widest">Teléfono</label>
                  <input type="tel" name="telefono" placeholder="+00 000 000 0000" value={formData.telefono} onChange={handleChange}
                    className="bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3 text-white text-sm placeholder-white/20 outline-none transition-all duration-300 focus:border-neutral-600" />
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-white/40 text-xs font-medium uppercase tracking-widest">Servicio de interés <span className="text-[#c5362e]">*</span></label>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-white/40 text-xs font-medium uppercase tracking-widest">Mensaje <span className="text-[#c5362e]">*</span></label>
                ...
                <button type="submit" disabled={sending}
                  className="mt-2 w-full relative text-white font-montserrat font-semibold text-sm px-6 py-4 rounded-xl transition-all duration-300 hover:opacity-90 flex items-center justify-center gap-2 disabled:opacity-60 bg-gradient-to-r from-[#009fe3] to-[#599ddf]">
                  {sending ? (
                    <>
